...
Features:
--------------------------------------------------------------------------------
With this code a ClipperCMS template variable could be transformed into a sortable multi item list
  
Usage:
--------------------------------------------------------------------------------
1. Create a new template variable with imput type *custom input* (if you name this template variable *multidemo* it will use the multidemo config file)
2. Insert the following code into the *input option values* 
```
@INCLUDE/assets/tvs/multitv/multitv.customtv.php
```

3. If you want to use multiTV with YAMS you have to patch yams.plugin.inc.php according to the instructions on https://github.com/Jako/multiTV/issues/9#issuecomment-6992127 
4. If you are updating from 1.4.10 and below you could install the updateTV snippet (see part 4) and modify the data in your multiTVs to the new format. Since the custom tv and the snippet code supports the old and new format, this is only nessesary, if you want to add/remove columns in your multiTVs or if you want to sort the output by a column. 
5. If you want to use PHx with multiTV you have to modify the PHx plugin code a bit:
...
type | type of the input (could be set to all ClipperCMS input types - without url and richtext - and thumb for thumbnail display of image tvs - see images config for thumb) | text
elements | could be set according to the input option values of a normal ClipperCMS template variable i.e. for a dropdown with all documents in the ClipperCMS root: ``@SELECT `pagetitle`, `id` FROM `clpr_site_content` WHERE parent = 0 ORDER BY `menuindex` ASC`` | -
...
* Supported ClipperCMS input types: text, rawtext, email, number, textareamini, textarea, rawtextarea, htmlarea, date, dropdown, listbox, listbox-multiple, checkbox, option, image, file
...
* In the editing layer the ClipperCMS input type richtext is possible.
...
Notes:
--------------------------------------------------------------------------------
1. The JSON string the multitv is converted to starts with `[[` and ends with `]]` so the ClipperCMS parser thinks it contains a snippet and you can't place the template variable directly in the template.
...
3. ManagerManager expects a custom tv field to be an input tag. Because of single and double quote issues the field containing the multiTV value is a textarea.
